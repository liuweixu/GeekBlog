    ...
    <el-dialog :visible.sync="addOrEdit" width="40%" top="3vh">
      ...
      <el-form label-width="80px" size="medium" :model="article">
        ...
        <el-form-item label="上传封面">
          <el-tooltip class="item" effect="dark" content="为空会采用默认全局接口随机图片！" placement="top">
            <el-upload
                class="upload-cover"
                drag
...
export default {
  ...
  methods: {
    ...
    handleChange(file, fileList) {
      ...
      // console.log(this.article.fileAttach.map(m=>{return m.response.data}))
      this.article.fileAttach = fileList.slice(-3);
    },
    saveArticleDraft() {
      if (this.article.articleTitle.trim() == "") {
        this.$message.error("文章标题不能为空");
    ...
    saveOrUpdateArticle() {
      ...
      if (this.article.fileAttach != null && this.article.fileAttach.length > 0){
        this.article.fileAttach = this.article.fileAttach.map(m=>{return m.response.data})
      }
      ...
      // }
      // if (this.article.articleCover.trim() == ""){
      //     this.article.articleCover = "https://api.r10086.com/%E6%A8%B1%E9%81%93%E9%9A%8F%E6%9C%BA%E5%9B%BE%E7%89%87api%E6%8E%A5%E5%8F%A3.php?%E8%87%AA%E9%80%82%E5%BA%94%E5%9B%BE%E7%89%87%E7%B3%BB%E5%88%97=%E5%8E%9F%E7%A5%9E";
      // }
      this.axios.post("/api/admin/articles", this.article).then(({data}) => {
        if (data.flag) {
          if (this.article.id === null) {
