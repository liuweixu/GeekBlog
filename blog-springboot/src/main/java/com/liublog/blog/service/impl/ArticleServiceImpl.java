...
import com.liublog.blog.constant.RedisPrefixConst;
import com.liublog.blog.dto.*;
import com.liublog.blog.exception.BizException;
import com.liublog.blog.service.*;
import com.liublog.blog.strategy.context.SearchStrategyContext;
import com.liublog.blog.strategy.context.UploadStrategyContext;
import com.liublog.blog.dao.ArticleDao;
import com.liublog.blog.dao.ArticleTagDao;
import com.liublog.blog.dao.CategoryDao;
import com.liublog.blog.dao.TagDao;
import com.liublog.blog.util.*;
...
import com.liublog.blog.enums.FileExtEnum;
import com.liublog.blog.enums.FilePathEnum;
import com.liublog.blog.enums.YesOrNoEnum;
import com.liublog.blog.service.*;
import com.liublog.blog.vo.*;
...
import static com.liublog.blog.common.ResultCode.*;
import static com.liublog.blog.enums.ArticleStatusEnum.DRAFT;
import static com.liublog.blog.enums.ArticleStatusEnum.PUBLIC;
...
public class ArticleServiceImpl extends ServiceImpl<ArticleDao, Article> implements ArticleService {
    ...
    @Override
    public ArticleDTO getArticleByIds(Integer articleId) {
        // 查询推荐文章
    ...
    public void saveArticleLike(Integer articleId, HttpServletRequest request) {
        ...
        // 判断是否点赞
        String articleLikeKey = RedisPrefixConst.ARTICLE_USER_LIKE +sessionId;
...
//                article.setArticleCover(webConfig.get().getArticleCover()+"&time="+CommonUtils.getRandomCode());
                if(StrUtil.isBlank(webConfig.get().getArticleCover())){
                    article.setArticleCover(RandomPictureUtils.getPictureUrl());
                }else{
...
//        String articleBackId = "articleBack_" + articleId;
//        if(redisService.get(articleBackId) != null){
//
//        }
        // 查询文章信息
    ...
    public Result reptile(String url) {
        try {
            ...
            String strResult = restTemplate.getForObject(CommonConst.IMG_URL_API, String.class);
...
//                    .articleContent(newContent)
                    .categoryId(CommonConst.OTHER_CATEGORY_ID)
    ...
    public Result randomImg() {
        //文章封面图片 由https://api.btstu.cn/该网站随机获取
        String result = restTemplate.getForObject(CommonConst.IMG_URL_API, String.class);
