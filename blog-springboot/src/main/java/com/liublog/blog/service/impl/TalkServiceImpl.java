...
import com.liublog.blog.service.TalkService;
import com.liublog.blog.dao.CommentDao;
import com.liublog.blog.dto.CommentCountDTO;
import com.liublog.blog.dto.TalkBackDTO;
import com.liublog.blog.dto.TalkDTO;
import com.liublog.blog.entity.Talk;
import com.liublog.blog.service.RedisService;
import com.liublog.blog.dao.TalkDao;
...
import static com.liublog.blog.enums.TalkStatusEnum.PUBLIC;
...
public class TalkServiceImpl extends ServiceImpl<TalkDao, Talk> implements TalkService {
    ...
    public List<String> listHomeTalks() {
        ...
        return talkDao.selectList(new LambdaQueryWrapper<Talk>()
                        ...
                        .last("limit 10"))
                .stream()
//                .map(item -> item.getContent().length() > 50 ? HTMLUtils.deleteHMTLTag(item.getContent().substring(0, 50)) : HTMLUtils.deleteHMTLTag(item.getContent()))
                .map(item ->{
                    String content;
                    if (item.getContent().length()>45){
                        content = HTMLUtils.deleteHMTLTag(item.getContent().substring(0, 45)+"...");
                    }else {
                        content = HTMLUtils.deleteHMTLTag(item.getContent());
                    }
                    if (content.contains("\n")){
